...
import { Camera, RefreshCcw, CameraOff, Download, Loader2, Sparkles, AlertTriangle } from 'lucide-react';
...
const GlassesModel = ({ faceData, modelPath }: { faceData: FaceData; modelPath: string }) => {
  ...
  useEffect(() => {
    const initTracker = async () => {
      try {
        ...
        setError("Failed to load face tracking models.");
        setLoading(false);
      }
    };
    initTracker();
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        setError("Camera permission denied or not available. Please enable camera access and try again.");
  ...
  const detectFrame = () => {
    if (videoRef.current && landmarkerRef.current && videoRef.current.readyState >= 2) {
      ...
      if (results.faceLandmarks && results.faceLandmarks.length > 0) {
        ...
        setFaceData({
          position: new THREE.Vector3((0.5 - nose.x) * 10, (0.5 - nose.y) * 10, -5),
          rotation: new THREE.Euler(
            (nose.y - 0.5) * 0.5, 
            (0.5 - nose.x) * 0.5, 
            0
    ...
    <div className="relative bg-slate-950 overflow-hidden flex flex-col items-center justify-center font-sans pt-20 ">
      ...
      <div className="relative z-10 text-center mb-8 px-4">
        <h1 className="text-4xl md:text-6xl font-black text-white mb-4 tracking-tight">
          Try Your <span className="text-transparent bg-clip-text bg-linear-to-r from-[#FFA500] to-[#FFD700]">Glasses in 3D</span>
        </h1>
        <p className="text-gray-400 max-w-lg mx-auto text-sm md:text-base">
          Experience our virtual fitting room. Align your face with the camera to see the magic.
        </p>
      </div>
      ...
      <div className="relative w-full max-w-4xl h-screen bg-black rounded-3xl overflow-hidden shadow-2xl border border-white/10 group">
        ...
        {loading && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-900/80 backdrop-blur-md">
            ...
            <p className="text-white font-bold tracking-widest animate-pulse">INITIALIZING AI TRACKER...</p>
          </div>
        )}
          ...
          <div className="absolute inset-0 z-20 pointer-events-none">
            <Canvas>
              ...
              <Suspense fallback={null}>
                <GlassesModel faceData={faceData} modelPath={models[modelIndex]} />
              </Suspense>
            </Canvas>
          </div>
        )}
        ...
        {faceData.detected && (
          <div className="absolute top-6 left-6 flex items-center gap-2 px-4 py-2 bg-green-500/20 backdrop-blur-md border border-green-500/50 rounded-full animate-in fade-in zoom-in duration-300">
            ...
            <span className="text-green-400 text-xs font-black uppercase tracking-widest">Face Aligned</span>
          </div>
        )}
      </div>
        ...
        {models.map((_, idx) => (
          <button
            ...
            className={`w-20 h-20 rounded-2xl border-2 transition-all shrink-0 bg-white/5 backdrop-blur-md ${
              modelIndex === idx ? 'border-[#FFA500] scale-110 bg-white/10' : 'border-white/10'
            }`}
          >
            <span className="text-[#FFA500] text-xs font-bold uppercase tracking-tighter">Style {idx + 1}</span>
          </button>
        ))}
      </div>
      ...
      <div className="mt-8 mx-4 p-6 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl max-w-xl text-center">
        <p className="text-gray-400 text-sm italic">
          Tip: Ensure you are in a well-lit room and face the camera directly for the most accurate 3D placement.
        </p>
      </div>
    </div>
  );
